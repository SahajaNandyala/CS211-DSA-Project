...
while time <= time_limit and len(trees)>0:
    trees.sort(key=lambda x:((abs(x["x"]-current_x)+abs(x["y"]-current_y)),x["value"],-(x["full_profit"]+x["value"])/(abs(x["x"]-current_x)+abs(x["y"]-current_y)+x["d"])))
...
#-------------------cutting trees ------------
        ...
        time += trees[0]["d"] + trees[0]["position"] - current_x - current_y
